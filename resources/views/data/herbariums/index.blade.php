...
@section('content')
    ...
    <p>
        <a href="{{ route('herbariums.create') }}" class="btn btn-success">@lang('global.app_add_new')</a>
    </p>
                    ...
                    @if (count($users) > 0)
                        @foreach ($herbariums as $herbarium)
                            <tr data-entry-id="{{ $herbarium->id }}">
                                ...
                                <td>
                                    <a href="{{ route('herbariums.edit',[$herbarium->id]) }}" class="btn btn-xs btn-info">@lang('global.app_edit')</a>
                                    {!! Form::open(array(
                                        'style' => 'display: inline-block;',
                                        'method' => 'DELETE',
                                        'onsubmit' => "return confirm('".trans("global.app_are_you_sure")."');",
                                        'route' => ['herbariums.destroy', $herbarium->id])) !!}
